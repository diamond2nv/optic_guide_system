...
class TransformMatrixCalculator():
    ...
    def calculate_transform_matrixs(self, aruco_dict, markerLength_needle, markerLength_ref):
        """
        Calculates the transformation matrices for ArUco markers.

        Args:
            aruco_dict (cv2.aruco.Dictionary): ArUco dictionary used for marker detection.
            markerLength_needle (float): Length of the needle marker in meters.
            markerLength_ref (float): Length of the reference marker in meters.

        Returns:
            transform_matrix_needle (numpy.ndarray): Transformation matrix for the needle marker.
            transform_matrix_ref (numpy.ndarray): Transformation matrix for the reference marker.
        """
        transform_matrix_needle = None
        transform_matrix_ref = None
        ...
        try:
            if ids is not None and len(ids) > 0:
                for i in range(len(ids)):
                    # Check for needle marker
                    if ids[i] == 0:
                        rvec, tvec, markerPoints = aruco.estimatePoseSingleMarkers(
                            corners[i], markerLength_needle, intr_matrix, intr_coeffs)

                        rotation_matrix, _ = cv2.Rodrigues(rvec)
                        transform_matrix_needle = np.eye(4)
                        transform_matrix_needle[:3, :3] = rotation_matrix
                        transform_matrix_needle[:3, 3] = tvec[0]

                    # Check for reference marker
                    elif ids[i] == 8:
                        rvec, tvec, markerPoints = aruco.estimatePoseSingleMarkers(
                            corners[i], markerLength_ref, intr_matrix, intr_coeffs)

                        rotation_matrix, _ = cv2.Rodrigues(rvec)
                        transform_matrix_ref = np.eye(4)
                        transform_matrix_ref[:3, :3] = rotation_matrix
                        transform_matrix_ref[:3, 3] = tvec[0]

                return transform_matrix_needle, transform_matrix_ref
            else:
...
def transform_matrix_accuracy_5X5():
    aruco_dict_5 = aruco.getPredefinedDictionary(aruco.DICT_5X5_50)
    transform_matrix_calculator = TransformMatrixCalculator()
    while(1):
        start = time.time()
        transform_matrix_5_needle,transform_matrix_ref = transform_matrix_calculator.calculate_transform_matrixs(aruco_dict_5,NEEDLE_MARKER_LENGTH,REF_MARKER_LENGTH)
        print("transform_matrix_5_needle:",transform_matrix_5_needle)
